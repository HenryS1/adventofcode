...
(defun concatenate-numbers (one other)
  ...
  (the fixnum (+ (the fixnum (* (the (unsigned-byte 16) (number-shift other)) one)) other)))
               ...
               (if (null remaining)
                   ...
                   (when (<= current-total target)
                     (let ((next (car remaining)))
                       (declare (fixnum next))
                       (or (rec (cdr remaining) (* current-total next))
                           (when (<= (the fixnum (* current-total (number-shift next))) target)
                             (rec (cdr remaining) (concatenate-numbers current-total next)))
                           (rec (cdr remaining) (+ current-total next))))))))
      (rec (cdr numbers) (car numbers)))))
