...
(defun find-area-and-perimeter (start plots seen)
  (let ((stack (list start))
    ...
    (declare (optimize (speed 3)) (fixnum perimeter area rows columns) 
             ...
             (multiple-value-bind (row column) (floor current columns)
               (progn (when (and (> column 0)
                      ...
                      (when (and (> row 0)
                                 ...
                                 (not (gethash (the fixnum (- current columns)) seen)))
                        (push (the fixnum (- current columns)) stack)
                        (setf (gethash (the fixnum (- current columns)) seen) t))
                      (when (and (< row (- rows 1))
                                 (char= (aref locations (+ current columns)) plot)
                                 (not (gethash (the fixnum (+ current columns)) seen)))
                        (push (the fixnum (+ current columns)) stack)
...
(declaim (inline find-sides))
(defun find-sides (index locations rows columns)
  (declare (optimize (speed 3)) (fixnum index rows columns) ((simple-array character) locations))
  (multiple-value-bind (row column) (floor index columns)
      (let ((sides 0)
            (c (aref locations index)))
...
(defun find-area-and-sides (start plots seen)
  (let ((stack (list start))
        ...
        (rows (plots-rows plots))
        (columns (plots-columns plots)))
    (declare (optimize (speed 3)) (fixnum sides area rows columns) 
             ((simple-array character) locations))
    (setf (gethash start seen) t)
    (loop while stack
          for current = (pop stack)
