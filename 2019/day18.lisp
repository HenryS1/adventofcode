  ...
  (iter outer
        ...
        (iter (for nbr in (neighbours-full mp (car current) (cdr current)))
              (when (not (gethash nbr seen))
                ...
                (when (lower-case-p ch)
                  (in outer (collect (list ch (+ dist 1) next-keys next-doors))))))))

(defun make-reachable-graph (mp)
  (iter (with locs = (cons (find-start-coord mp) (find-key-coords mp)))
        (with graph = (make-hash-table))
        (for loc in locs)
        (setf (gethash (find-in-map mp loc) graph) (connected-through-doors loc mp))
        (finally (return graph))))
...
(defun a-star (mp)
  (let* ((graph (make-reachable-graph mp))
    ...
    (declare (optimize (speed 3) (safety 3))
             (fixnum num-keys))
    (cl-heap:enqueue q (list 0 nil start) 0)
;;    graph
    (iter (for elem = (cl-heap:dequeue q))
          (while elem)
          (for (moves-so-far unlocked current) = elem)
          (when (and (= (length unlocked) num-keys) (< moves-so-far best-dist))
            (format t "NEW CANDIDATE ~a~%" moves-so-far)
            (setf best-dist moves-so-far))
          (iter (for (other dist keys doors) in (gethash current graph))
                (for new-dist = (+ moves-so-far dist))
                (when (and (not (find other unlocked)) 
                           (< new-dist best-dist) (subsetp doors unlocked))
                  (for new-unlocked = (adjoin other unlocked))
                  ;; (format t "ELEM ~a~%" elem)
                  ;; (format t "ENQUEUEING ~a~%" (list new-dist
                  ;;                                   new-unlocked
                  ;;                                   other))
                  (cl-heap:enqueue q
                                   (list new-dist
                                         new-unlocked
                                         other)
                                   (+ new-dist (* 280 (- num-keys (length new-unlocked)))))))
          (finally (return best-dist)))))
