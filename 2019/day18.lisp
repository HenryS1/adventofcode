...
(defun make-state (position seen position-offset seen-mask)
  (logior (ash position position-offset) (logand (logior (ash 1 position) seen) seen-mask)))
(defun all-visited (state seen-mask) (= (logand state seen-mask) seen-mask))

(defun make-seen-mask (num-keys) (- (ash 1 num-keys) 1))
...
(defun find-dist-offset (num-keys) 
  (+ num-keys (find-sep-offset num-keys)))
  ...
  (let* ((seen-mask (make-seen-mask num-keys))
    ...
    (iter (for nbr in-vector (gethash (* 2 num-keys) distances))
          (bind (((:values sep loc) (decode-bits nbr sep-offset loc-mask))
                 (new-state (make-state loc 0 position-offset seen-mask)))
            (when (< loc num-keys)
    ...
    (iter (while (pq-nonempty pq))
          ...
          (when (all-visited state seen-mask)
          ...
          (iter (for nbr in-vector (gethash position distances))
                (bind (((:values sep loc) (decode-bits nbr sep-offset loc-mask))
                       (new-state (make-state loc (get-seen state seen-mask) position-offset seen-mask))
...
(defun find-dist-offset-par (num-keys) 
  (+ num-keys (* 4 (find-sep-offset num-keys))))
