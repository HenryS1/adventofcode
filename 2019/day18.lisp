...
(defun a-star (graph mp)
  (let ((q (make-pq (lambda (one other) (< (car one) (car other)))))
        (start (find-start-coord mp))
        (num-keys (length (find-keys mp))))
    (insert-pq (list 0 start nil (find-keys mp)) q)
    (iter (while (not (pq-empty q)))
          (for (moves-so-far current unlocked remaining) = (pop-pq q))
          (format t "UNLOCKED ~a MOVES ~a~%" unlocked moves-so-far)
          ;; (format t "CURRENT ~a MOVE ~a UNLOCKED ~a REMAINING ~a~%" 
          ;;         current moves-so-far unlocked remaining)
          ;; (format t "CURRENT ~a CH ~a UNLOCKED ~a MOVES ~a~%" current (find-in-map mp current)
          ;;         unlocked moves-so-far)
          ;; (format t "SEEN ~a~%" seen)
          (when (= (length unlocked) num-keys)
            (return-from a-star moves-so-far))
          (iter (for next in remaining)
                (for (next-moves next-unlocked next-remaining) =
                     (travel-between current next unlocked remaining graph mp))
;                (format t "OTHER ~a~%" other)
                (for ch = (find-in-map mp next))
                (when (or (lower-case-p ch) (find ch unlocked))
                  (insert-pq (list (+ moves-so-far next-moves) 
                                   next next-unlocked next-remaining) q))))))
