...
(defun find-connections (start keys-and-doors mp)
  (iter (with q = (make-queue (cons start 0)))
        (with seen = (alist-hash-table (list (cons start t)) :test 'equal))
        (with connections = (alist-hash-table 
...
;               (format t "REC ~a UNLOCKED ~a~%" current unlocked)
               (cond ((> moves-so-far bound) nil)
                     ((= (length unlocked) 26)
                      (when (< moves-so-far bound)
                        (setf bound moves-so-far)))
                     (t (iter (for (other . dist) in (gethash current graph))
                              (when (and (not (find other seen :test 'equal))
                                         (<= (+ dist moves-so-far) bound))
                                (for ch = (map-at mp (car other) (cdr other)))
;                                (format t "CH ~a~%" ch)
                                (when (or (lower-case-p ch) (find ch unlocked))
                                  (for next-remaining = (delete other remaining))
                                  (for next-unlocked = (if (lower-case-p ch)
                                                           (cons (char-upcase ch) unlocked)
                                                           unlocked))
                                  (rec other next-remaining 
                                       (+ moves-so-far dist) 
                                       next-unlocked
                                       (cons other seen)))))))))
      (rec (find-start-coord mp) (find-keys-and-doors mp) 0 nil nil))))
