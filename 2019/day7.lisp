...
(defun find-max-signal (codes)
  (iter outer 
        (with prog1 = (copy-seq codes))
        (for p1 from 0 to 4)
        (for o1 = (find-termination p1 0 prog1))
        (iter (for p2 from 0 to 4)
              (when (= p1 p2)
                (next-iteration))
              (with prog2 = (copy-seq codes))
              (for o2 = (find-termination p2 o1 prog2))
              (iter (for p3 from 0 to 4)
                    (when (or (= p3 p2) (= p3 p1))
                      (next-iteration))
                    (with prog3 = (copy-seq codes))
                    (for o3 = (find-termination p3 o2 prog3))
                    (iter (for p4 from 0 to 4)
                          (when (or (= p4 p3) (= p4 p2) (= p4 p1))
                            (next-iteration))
                          (with prog4 = (copy-seq codes))
                          (for o4 = (find-termination p4 o3 prog4))
                          (iter (for p5 from 0 to 4)
                                (when (or (= p5 p4) (= p5 p3) (= p5 p2) (= p5 p1))
                                  (next-iteration))
                                (with prog5 = (copy-seq codes))
                                (for o5 = (find-termination p5 o4 prog5))
...
(defun make-amp (id input-buffer output-buffer codes &optional (final-output nil))
  (declare (ignore id))
  (let ((i 0))
    (lambda () 
...
(defun run-feedback-loop (buf1 buf2 buf3 buf4 buf5 codes)
  (iter (with thruster = (vector 0))
        (with amp1 = (make-amp 1 buf1 buf2 (copy-seq codes)))
        (with amp2 = (make-amp 2 buf2 buf3 (copy-seq codes)))
        (with amp3 = (make-amp 3 buf3 buf4 (copy-seq codes)))
        (with amp4 = (make-amp 4 buf4 buf5 (copy-seq codes)))
        (with amp5 = (make-amp 5 buf5 buf1 (copy-seq codes) thruster))
...
(defun find-max-feedback (codes)
  (iter outer 
        (for p1 from 5 to 9)
        (iter (for p2 from 5 to 9)
              (when (= p1 p2)
                (next-iteration))
              (iter (for p3 from 5 to 9)
                    (when (or (= p3 p2) (= p3 p1))
                      (next-iteration))
                    (iter (for p4 from 5 to 9)
                          (when (or (= p4 p3) (= p4 p2) (= p4 p1))
                            (next-iteration))
                          (iter (for p5 from 5 to 9)
                                (when (or (= p5 p4) (= p5 p3) (= p5 p2) (= p5 p1))
                                  (next-iteration))
...
(defun answer-2 ()
  (find-max-feedback (read-program)))
