...
(defun affected-points ()
  (iter outer
        ...
        (iter (for y from 0 to 49)
              ...
              (enqueue-inp x computer)
              (enqueue-inp y computer)
;              (format t "INPUT ~a~%" (input computer))
              (compute computer)
;              (format t "OUTPUT ~a~%" (output computer))
...
(defun edges-from-file ()
  (let ((mp (map 'vector #'identity (iter (for line in-file "inputt" using #'read-line)
                                       (collect line)))))
    (iter (with tops = (make-hash-table :test 'equal))
          ...
          (iter (for ch in-vector row)
                (for c from 0)
                (when (and row-p (char= (aref row-p c) #\.) 
                           (char= (aref row c) #\#))
                  (setf (gethash c tops) r))
                (when (and row-p (char= (aref row c) #\.)
                           (char= (aref row-p c) #\#))
                  (setf (gethash c bottoms) (- r 1))))
...
(defun validate-edges (tops bottoms)
  (let ((program (read-program)))
    (iter (for (x y) in-hashtable tops)
;          (format t "X ~a Y ~a~%" x y)
          (if (or (/= (compute-coord program (cons x y)) 1)
                  (/= (compute-coord program (cons x (- y 1))) 0))
              (error (format nil "what happened ~a ~a~%" x y))))
    (iter (for (x y) in-hashtable bottoms)
          (if (or (/= (compute-coord program (cons x)) 1)
                  (/= (compute-coord program (cons (+ y 1) x)) 0))
              (error (format nil "what happened ~a ~a~%" x y))))))
...
;          (format t "X ~a~%" x)
          ...
          ;;   (format t "FOUND SOMETHING~%"))
          (format t "DIFF ~a~%" (- bottom-left top-right))
          (when (and top-left top-right bottom-left bottom-right
                     (>= (- bottom-left top-right) 99))
            (return (list x top-left top-right bottom-left bottom-right))))))
