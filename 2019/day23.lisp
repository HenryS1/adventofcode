...
;               (format t "PROCESS INPUT~%")
               (iter (for (i comp) in-hashtable computers)
;                     (format t "COMP ~a~%" i)
                     (when (empty (input comp))
;                       (format t "EMPTY INPUT~%")
                       (enqueue-inp -1 comp))
;                     (format t "COMP OUTPUT ~a~%" (output comp))
                     (compute comp)))
             (process-output ()
;               (format t "PROCESS OUTPUT~%")
;               (format t "OUTPUT ~a~%" (car global-output))
...
(defun answer-1 ()
  (let ((comp (make-computer (read-program))))
    (compute comp)
    (output comp)))
